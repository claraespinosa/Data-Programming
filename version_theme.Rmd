...
datatable(head(winners, n = nrow(winners)), options = list(pageLength = 5))
...
Frequency with which each party attains the second place when PSOE wins the elections.
...
datatable(head(second_after_PSOE , n = nrow(second_after_PSOE )), options = list(pageLength = 5))
...
datatable(head(second_after_PP , n = nrow(second_after_PP )), options = list(pageLength = 5))
```
...
```{r, fig.width=10, fig.height=6, out.width='100%', echo = FALSE, message = FALSE, warning = FALSE}
datatable(head(correlation_results_df , n = nrow(correlation_results_df )), options = list(pageLength = 5))
```

## QUESTION 4 | Visualization: correlations per party
Relationship between participation  and party votes 
...
datatable(head(fraud, n = nrow(fraud)), options = list(pageLength = 5))
...
```{r, fig.width=10, fig.height=6, out.width='100%', echo = FALSE, message = FALSE}
ggplot(correlation_results_df, aes(x = Partido, y = Correlacion)) +
...
datatable(head(rural_winner , n = nrow(rural_winner)), options = list(pageLength = 2))
```

## QUESTION 5 | PART 2: Visualization Correlations
```{r, fig.width=10, fig.height=6, out.width='100%', echo = FALSE, message = FALSE}
library(forcats)
...
grafico_rural <- ggplot(rural_winner, aes(x = date, y = percentage, fill = Partidos)) +
  ...
  scale_fill_manual(values = colores_partidos) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  scale_x_discrete(breaks = fechas_unicas) +
...
which had notably higher errors. 

---

In model error calibration, identifying variables 
...
How is the accuracy of the pollsters as the date approaches?
